...
class _SalesEditorScreenState extends ConsumerState<SalesEditorScreen>
  ...
  List<_SaleProduct> _mapSaleProducts(List<Map<String, dynamic>> items) {
    return items.asMap().entries.map((entry) {
      ...
      final salePrice = _pickDouble(
          item, ['sale_price', 'salePrice', 'discount_price', 'discountPrice']);
      var original = _pickDouble(
          item, ['original_price', 'originalPrice', 'price'],
  ...
  Future<List<Map<String, dynamic>>> _fetchSaleProductRows(
      String saleId) async {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (!r.success || r.data == null) {
        throw StateError(r.error?.message ?? 'Failed to load sale');
      }
      final payload = r.data;
      if (payload is! Map<String, dynamic>) {
        throw const FormatException('Invalid sale payload');
      }
      final raw =
          payload['sale'] ?? payload['item'] ?? payload['data'] ?? payload;
  ...
  Future<void> _addCatalogProduct(Map<String, dynamic> raw) async {
    ...
    if (id.isEmpty) {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Product has no id/sku')));
      return;
    }
    if (_products.any((p) => p.productId == id)) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Product already in this sale')));
      return;
    }
    ...
    final image = _pickString(
        raw, ['thumbnail', 'thumbnail_url', 'image', 'image_url', 'imageUrl'],
  ...
  Future<void> _openAddProductsSheet() async {
    final existing =
        _products.map((p) => p.productId).where((id) => id.isNotEmpty).toSet();
    ...
    try {
      ...
      if (widget.isCreate) {
        ...
        for (final p in _products) {
          ...
          if (!ar.success) {
            throw StateError(ar.error?.message ?? 'Add product failed');
          }
        }
      } else {
        final updated =
            await api.updateSale(widget.saleId!, _saleMetaPayload());
        if (!updated.success) {
          throw StateError(updated.error?.message ?? 'Save failed');
        }
        for (final p in _products) {
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(p.name,
                              style: GoogleFonts.inter(
                                  fontWeight: FontWeight.w700)),
                          Text('Original: ${p.original.toStringAsFixed(2)}',
                              style: GoogleFonts.inter(
                                  fontSize: 12,
                                  color: theme.colorScheme.onSurfaceVariant)),
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                final id = _id(p);
                final inSale =
                    id.isNotEmpty && widget.existingProductIds.contains(id);
